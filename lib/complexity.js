...
function analyzeComplexity(content, fileExtension) {
  ...
  } else if (['.kt', '.kts'].includes(fileExtension)) {
    analyzeKotlinComplexity(content, results);
...
function analyzeKotlinComplexity(content, results) {
  // Match Kotlin function declarations (fun keyword)
  const functionRegex = /(?:public|private|protected|internal)?\s*(?:open|override|suspend)?\s*fun\s+(\w+)\s*\([^)]*\)\s*(?::\s*\w+)?\s*[{=]/gms;
  let match;
  ...
  while ((match = functionRegex.exec(content)) !== null) {
    ...
    // For single-expression functions (= instead of {), extract until the next function or end
    if (content[match.index + match[0].length - 1] === '=') {
      // Single expression function - find the end (next fun keyword or end of content)
      const nextFunMatch = /\bfun\s+/.exec(content.substring(startPos + match[0].length));
      const endPos = nextFunMatch 
        ? startPos + match[0].length + nextFunMatch.index 
        : content.length;
...
function analyzePHPComplexity(content, results) {
  ...
  // Uses 'ms' flags to handle newlines between function name and parentheses
  // Supports multiline parameter lists and various PHP syntaxes
  const functionRegex = /(?:public|private|protected)?\s*(?:static\s+)?function\s+(\w+)\s*\([^\)]*?\)\s*(?:\{|:)/gms;
  let match;
