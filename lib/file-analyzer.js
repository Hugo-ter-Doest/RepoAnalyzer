...
function analyzeFile(filePath, repoPath) {
  try {
    ...
    if (['.js', '.jsx', '.ts', '.tsx', '.mjs', '.cjs'].includes(ext)) {
      analyzeJavaScriptTypeScript(content, results);
    } else if (ext === '.py') {
      analyzePython(content, results);
    } else if (ext === '.java') {
      analyzeJava(content, results);
    } else if (['.kt', '.kts'].includes(ext)) {
      analyzeKotlin(content, results);
    } else if (ext === '.cs') {
      analyzeCSharp(content, results);
    } else if (['.cpp', '.cc', '.cxx', '.hpp', '.h', '.c'].includes(ext)) {
      analyzeCppC(content, results);
    } else if (ext === '.go') {
      const goModulePath = getGoModulePath(repoPath);
      analyzeGo(content, results, goModulePath);
    } else if (ext === '.rb') {
      analyzeRuby(content, results);
    } else if (ext === '.php') {
      analyzePHP(content, results);
    } else if (ext === '.rs') {
      analyzeRust(content, results);
    } else if (['.html', '.htm'].includes(ext)) {
      analyzeHTML(content, results);
      results.isSpecification = true;
    } else if (ext === '.css') {
      analyzeCSS(content, results);
      results.isSpecification = true;
    } else if (ext === '.xml') {
      analyzeXML(content, results);
      results.isSpecification = true;
    } else if (['.json'].includes(ext)) {
      analyzeJSON(content, results);
      results.isSpecification = true;
    } else if (['.yaml', '.yml'].includes(ext)) {
      analyzeYAML(content, results);
      results.isSpecification = true;
    }

    // Keep function totals for code files (complexity is handled by Lizard)
    if (!results.isSpecification) {
      results.functionsCount = results.publicMethods;
    }
...
function analyzeJavaScriptTypeScript(content, results) {
  // Count classes
...
function analyzePython(content, results) {
  // Count classes
...
function analyzeJava(content, results) {
  // Count classes
...
function analyzeKotlin(content, results) {
...
function analyzeCSharp(content, results) {
  // Count classes
...
function analyzeCppC(content, results) {
  // Count classes
  const classMatches = content.match(/\bclass\s+\w+/g);
  results.classes = classMatches ? classMatches.length : 0;
...
function analyzeGo(content, results, goModulePath = null) {
...
function analyzeRuby(content, results) {
...
function analyzePHP(content, results) {
  // Count classes
  const classMatches = content.match(/\bclass\s+\w+/g);
  results.classes = classMatches ? classMatches.length : 0;
...
function analyzeRust(content, results) {
...
function analyzeHTML(content, results) {
}

function analyzeCSS(content, results) {
}

function analyzeXML(content, results) {
}

function analyzeJSON(content, results) {
}

function analyzeYAML(content, results) {
}
